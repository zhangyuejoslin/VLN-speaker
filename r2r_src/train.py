...
def train(train_env, tok, n_iters, log_every=2000, val_envs={}, aug_env=None):
    ...
    speaker = None
    if args.self_train:
        speaker = Speaker(aug_env, listner, aug_env.tok)
...
def train_val_augment(test_only=False):
    ...
    tok_speaker = Tokenizer(vocab=vocab, encoding_length=args.maxInput)
    ...
    aug_env   = R2RBatch(feat_dict, batch_size=args.batchSize, splits=[aug_path], tokenizer=tok_speaker, name='aug')
