...
class LXRTXLayer(nn.Module):
    ...
    def self_att(self, visn_input, visn_attention_mask, lang_input=None, lang_attention_mask=None):
    ...
    def output_fc(self, visn_input, lang_input=None):
        ''' Feed forward '''
    ...
    def forward(self, lang_feats, lang_attention_mask,
                      visn_feats, visn_attention_mask, tdx, mode=None):
        ...
        if not mode:
            ...
            _, visn_att_output, cross_attention_scores = self.cross_att(lang_feats[:, 1:, :], lang_attention_mask[:, :, :, 1:], visn_att_output, state_vis_mask)
            # visn_attn_output: 4*5*768
            # cross_attention_scores: 4*12*5*79
            language_attention_scores = cross_attention_scores[:, :, 0, :]
            # language_attention_scores: 4*12*79
            state_visn_att_output = self.self_att(visn_input=visn_att_output, visn_attention_mask=state_vis_mask)
            # state_visn_att_output[0]4*5*768 state_visn_att_output[1] 4*12*5*5
            state_visn_output = self.output_fc(state_visn_att_output[0])
            #state_visn_output 4*5*768

            visn_att_output = state_visn_output[:, 1:, :]
            # visn_att_output 4*4*768
            lang_att_output = torch.cat((state_visn_output[:, 0:1, :], lang_feats[:,1:,:]), dim=1)
            # lang_att_output 4*80*768
            visual_attention_scores = state_visn_att_output[1][:, :, 0, 1:]
            # visual_attention_scores 4*12*4
            return lang_att_output, visn_att_output, language_attention_scores, visual_attention_scores

        else:
            ## input vision at first, output language first
            lang_att_output = lang_feats
            visn_att_output = visn_feats
            lang_att_output, visn_att_output, _ = self.cross_att(lang_att_output, lang_attention_mask,
                                                            visn_att_output, visn_attention_mask)
            lang_att_output, visn_att_output = self.self_att(visn_att_output, visn_attention_mask,
                                                            lang_input=lang_att_output, lang_attention_mask=lang_attention_mask)
            lang_output, visn_output = self.output_fc(visn_att_output[0], lang_att_output[0])
...
class VLNBert(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None,
        attention_mask=None, lang_mask=None, vis_mask=None, position_ids=None, head_mask=None, img_feats=None, labels=None):
        ...
        if mode == 'language':
            ...
            #img_embedding_output = self.only_img_encoder(img_feats[:,:,:2048])
            ...
            visn_output = img_embedding_output
            for tdx, layer_module in enumerate(self.addlayer):
                lang_output, visn_output = layer_module(lang_output, text_mask, visn_output, img_mask, tdx, mode=mode)
