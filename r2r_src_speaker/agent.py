...
class Seq2SeqAgent(BaseAgent):
    ...
    def _candidate_variable(self, obs):
        ...
        #candidate_obj_feat = np.zeros((len(obs), max(candidate_leng), 36, 2048), dtype=np.float32) #YZ
    ...
    def get_input_feat(self, obs):
        ...
        return input_a_t, candidate_feat, candidate_leng#YZ
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        if speaker is not None:         # Trigger the self_train mode!
            ...
            boss = np.ones((batch_size, 1), np.int64) * speaker.tok.vocab['[CLS]']  # First word is <BOS>
            insts = np.concatenate((boss, insts), 1)
            for i, (datum, inst) in enumerate(zip(batch, insts)):
                if inst[-1] != speaker.tok.vocab['[PAD]']: # The inst is not ended!
                    inst[-1] = speaker.tok.vocab['[SEP]']
                datum.pop('instructions')
                datum.pop('instr_encoding')
                datum['instructions'] = speaker.decode_sentence(inst)
        ...
        for t in range(self.episode_len):
            ...
            input_a_t, candidate_feat, candidate_leng = self.get_input_feat(perm_obs) #YZ 
